...
jointure play dispatched in breeding. Education pleasure fanny debating off
...
rozdelena_slova = list()

for slovo in text.upper().split():
    rozdelena_slova.append(slovo.strip(",.:"))

vyskyt_slov = dict()  # "do": 4, "sex": 2, ..

for slovo in rozdelena_slova:  # "do", "sex", "lasted", ..
    if slovo in vyskyt_slov:
        vyskyt_slov[slovo] = vyskyt_slov[slovo] + 1
    else:
        vyskyt_slov[slovo] = 1

top_3 = (sorted(vyskyt_slov.values(), reverse=True)[:3]) # 8, 8, 6

for vyskyt in top_3:  # 8, 8, 6
    for klic, hodnota in vyskyt_slov.items():
        if hodnota == vyskyt: # "HE": 8, "IN": 8, "HE": 8, "IN": 8, "YE": 6
